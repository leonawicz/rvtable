...
and any categrorical variables whose levels propogate any ID columns.
...
we arrive at the more common ways to constuct an rvtable: from and existing data frame.
...
all columns present that are not one of the two values or probabilites columns.
...
`rvtable` constructor to also apply any maginalizing operations if it is passed a data frame that has no or only
partial grouping.
...
Returning to the data frame from eariler, sampling is performed by group.
...
It is important to be aware of the level of possible signal degredation that can occur with some combination of:
...
* the number of times a desnity is re-estimated from a sample, such as happens when computing marginal distributions,
...
if is not difficult to force retention of the original desnity estimation within acceptable tolerances for
...
A number of resampling and re-estimation cycles are repeated in each case and signal degredation is plotted for select iterations.
...
one that is ideally revisted as seldom as possible for subsequent analyses.
It is much easier to revisit a smaller, derved data set that is still statistically sufficient for the analysis at hand.
...
all the while wishing they could have forseen the need last time for whatever they are accessing it for this time around.

`rvtable` bridges this gap for many use cases like this, where the happy medium, the goldilocks zone,
is to model and store empircal estimates of probability distributions from high resolution source data
or massive sample or populuation data. Downstream analyses can depend on such a
