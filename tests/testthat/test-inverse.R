...
test_that("inverse_pmf on continuous RV returns correctly", {
  ...
  purrr::walk2(ylist, c("id1", "id2", "id3"), ~expect_identical(valcol(.x), .y))
  purrr::walk(ylist, ~expect_identical(probcol(.x), "Prob1"))
...
test_that("inverse_pmf on discrete RV returns correctly", {
  ...
  purrr::walk2(ylist, c("id1", "id2", "id3", rep("id1", 3)), ~expect_identical(valcol(.x), .y))
  purrr::walk2(ylist, rep(c("Prob1", "Prob"), each=3), ~expect_identical(probcol(.x), .y))
...
test_that("inverse_pmf returns consistent, correct values", {
  ...
  purrr::walk2(ylist2, rep("id", 4), ~expect_identical(valcol(.x), .y))
  purrr::walk2(ylist2, rep("Prob", 4), ~expect_identical(probcol(.x), .y))
